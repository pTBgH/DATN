...
# Pick the registry the host (srv01) will push images to. KIND uses a local
# docker-compose registry on localhost:5000. VM resolves the in-cluster
# registry NodePort + node hostname (deployed via 12-docker-registry.yaml).
if is_kind_mode; then
  echo ""
  echo "📦 KIND mode — using host-side docker-compose registry at http://localhost:5000"
  ...
  echo ""
  echo "📦 VM mode — using in-cluster Docker registry (12-docker-registry.yaml)"
...
# Determine chart registry FQDN used by Helm charts. This becomes the prefix
# rendered into each Deployment's `image:` field — kubelets pull from there.
#
# KIND: chart values default to 172.17.0.1:5000 (docker0 bridge into the
#       host-side docker-compose registry started above).
# VM:   use the same endpoint as REGISTRY_HOST (the in-cluster registry's
#       NodePort + node-IP/hostname). One endpoint for both push and pull
#       so every kubelet reaches it the same way the host does.
...
# In VM mode the chart's default registry.url (172.17.0.1:5000, kind-era) is
# wrong: it points to the docker0 bridge, which doesn't exist on the real VMs.
# Write a one-shot override values file and append it to every release so the
# rendered Deployment image: prefix matches REGISTRY_HOST (the in-cluster
# registry NodePort + node hostname we just resolved).
HELMFILE_EXTRA_ARGS=()
if is_vm_mode; then
  HELMFILE_REGISTRY_OVERRIDE="/tmp/zta-vm-registry-override.yaml"
  cat > "$HELMFILE_REGISTRY_OVERRIDE" <<EOF
# Auto-generated by 03-deploy-microservices.sh in VM mode.
# Overrides chart default (172.17.0.1:5000) so kubelets pull from the
# in-cluster registry we resolved at runtime.
registry:
  url: "${CHART_REGISTRY}"
EOF
  echo "   VM mode: appending registry override → $HELMFILE_REGISTRY_OVERRIDE (registry.url=$CHART_REGISTRY)"
  HELMFILE_EXTRA_ARGS+=(--values "$HELMFILE_REGISTRY_OVERRIDE")
fi
...
set +e
helmfile --selector namespace=job7189-apps apply "${HELMFILE_EXTRA_ARGS[@]}" 2>&1 | tee /tmp/helmfile-output.log
