...
| # | Check | Cách verify | Pass nếu |
|---|-------|-------------|----------|
| D1 | Repo cloned trên cả 4 VM | `ls ~/DATN/doc/migration/scripts` | thấy 6 file `.sh` |
| D2 | Latest commit | `cd ~/DATN && git log --oneline -1` | matches origin/main |
| D3 | `config.env` đã được tạo và edit | `cat ~/DATN/doc/migration/scripts/config.env` | TAILNET_DOMAIN, TS_AUTHKEY (hoặc thủ công) đã set |
| D4 | Scripts có executable bit | `ls -la ~/DATN/doc/migration/scripts/*.sh` | `-rwxr-xr-x` |

Nếu D4 fail:
```bash
chmod +x ~/DATN/doc/migration/scripts/*.sh
```
...
sudo ZTA_DRY_RUN=1 HOSTNAME_OVERRIDE=$(hostname) -E \
  bash doc/migration/scripts/01-host-prep.sh
