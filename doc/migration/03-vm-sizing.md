...
| `7189srv04` | Ubuntu  | **4.0 GB** | 2 | 60 GB ext4 | data tier always-on (RAM-trimmed) |
...
### 7189srv04 (4.0 GB) — **RAM-trimmed profile**
...
| ~~Tetragon (DS)~~ | — | — | **SKIP srv04** (toleration: not on data tier) |
| Filebeat (DS) | 64 Mi | 128 Mi | giảm |
| spire-agent + spiffe-csi (DS) | 96 Mi | 192 Mi | giảm |
| node-exporter (DS) | 32 Mi | 64 Mi | giữ |
| OS Debian 13 reserved + system-reserved + kube-reserved | ~512 Mi | — | |
| **Total** | **~1.9 GB** | **~3.2 GB** | |
...
- Trade-off ngược: nếu cần persist log >6h, bump srv04 lên 5-6 GB và
  revert ES + Prom về srv04 (xem §8 fallback).
...
| Option | Action | Tác động |
|--------|--------|----------|
| E | Bump srv04 lên 5-6 GB | Cần đảm bảo Ubuntu host còn ≥3 GB cho app khác (browser, IDE) |
...
`RESOURCE_PROFILE=normal` (default) áp dụng giá trị cũ (giả định srv04
6 GB).
