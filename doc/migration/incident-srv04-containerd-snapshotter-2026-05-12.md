...
- `doc/migration/12-runbook-recovery.md`: new §13 "Containerd snapshotter corrupt after unclean shutdown" with the exact recovery procedure.
- `doc/migration/PROGRESS.md`: new install-progress tracker so the next session knows exactly which phase srv04 was at when this happened.
