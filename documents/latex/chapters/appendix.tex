...
\begin{itemize}
    \item \textbf{Cơ chế hoạt động}: Cổng giao tiếp vận hành ở chế độ khai báo không dùng cơ sở dữ liệu (DB-less mode) thông qua tệp cấu hình nguồn \texttt{kong.yml}. Cổng thực hiện đối chiếu mã thông báo định danh JWT từ trường tiêu đề \texttt{Authorization} trong yêu cầu HTTP của người dùng bằng khóa công khai tương ứng. Hệ thống tích hợp một trình cắm tiền chức năng (pre-function plugin) viết bằng ngôn ngữ Lua để giải mã mã thông báo JWT và gửi yêu cầu xác thực đến dịch vụ chính sách OPA nhằm quyết định cho phép hoặc từ chối truy cập trước khi chuyển tiếp yêu cầu đến các vi dịch vụ đích.
...
\begin{itemize}
    \item \textbf{Cơ chế hoạt động}: Keycloak thực hiện xác thực người dùng dựa trên vùng quản lý định danh (realm) được cấu hình. Sau khi xác thực thành công, Keycloak thực hiện ký số mã thông báo truy cập (JWT) bằng khóa riêng tư. Khóa công khai dùng để xác minh chữ ký được cung cấp qua điểm cuối JWKS, cho phép cổng giao tiếp Kong và các vi dịch vụ tải về để đối chiếu cục bộ.
...
\subsection{Các vi dịch vụ nghiệp vụ}
Các vi dịch vụ nghiệp vụ được cấu hình chạy song song với các container phụ trợ trong cùng một Pod để hỗ trợ cập nhật thông tin bảo mật tự động.
\begin{itemize}
...
\begin{minted}[fontsize=\footnotesize]{bash}
# Triển khai 7 vi dịch vụ nghiệp vụ vào không gian tên job7189-apps
...
Để phân tích luồng dữ liệu hoặc xử lý sự cố kết nối giữa các vi dịch vụ, quản trị viên sử dụng giao diện đồ họa của Hubble theo các bước sau:
\begin{enumerate}
