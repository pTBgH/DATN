...
\textbf{Điểm cần nhấn mạnh} là lưu lượng nội bộ không đồng nghĩa với lưu lượng đáng tin cậy. Khi một dịch vụ bị chiếm quyền, chính các kết nối nội bộ hợp lệ có thể bị lợi dụng để dò quét, thu thập dữ liệu và di chuyển ngang \parencite{nist800204_microservices,mitre_t1046,mitre_t1021}.
...
Trong trường hợp một dịch vụ bị thỏa hiệp, tác nhân tấn công có thể lợi dụng các kết nối hợp lệ đã tồn tại để thực hiện trinh sát, thu thập thông tin hoặc di chuyển ngang sang các thành phần khác. Các kỹ thuật dò quét dịch vụ và di chuyển ngang như Network Service Discovery và Remote Services được MITRE ghi nhận là những hành vi phổ biến trong giai đoạn hậu khai thác \parencite{mitre_t1046,mitre_t1021}. Mức độ rủi ro càng gia tăng khi các dịch vụ \textbf{\textit{mặc định tin cậy lẫn nhau}} chỉ vì cùng nằm trong một cụm Kubernetes hoặc cùng thuộc một vùng mạng.

Một giải pháp thường được áp dụng là mã hóa toàn bộ lưu lượng nội bộ bằng cơ chế xác thực hai chiều \parencite{nist800204a_service_mesh,nist800204b_abac_microservices}. Tuy nhiên, \textit{mã hóa chỉ bảo vệ kênh truyền}; nó không tự trả lời câu hỏi dịch vụ nào được phép gọi dịch vụ nào, với phương thức nào và trong ngữ cảnh nào. Do đó, hệ thống cần đạt được sự cân bằng giữa yêu cầu bảo mật, khả năng mở rộng và hiệu năng vận hành.
...
\textbf{Điểm cần nhấn mạnh} là an toàn trước triển khai chưa đủ để bảo vệ hệ thống lúc đang chạy. Một image có thể vượt qua bước quét ban đầu nhưng vẫn sinh hành vi bất thường khi ứng dụng xử lý dữ liệu thật \parencite{nist800204c_devsecops_microservices,tetragon_docs}.
...
\textbf{Điểm cần nhấn mạnh} là workload không chỉ cần chạy được, mà còn phải chứng minh được \textit{nguồn gốc} và \textit{tính toàn vẹn} trước khi được phép đi vào cluster \parencite{cncf_swp,newman2022sigstore,sigstore_docs}.

Mỗi workload triển khai trong Kubernetes thường được xây dựng từ nhiều thành phần phần mềm khác nhau như hệ điều hành nền, thư viện phụ thuộc, container image và các gói mã nguồn từ bên thứ ba. Điều này khiến \textbf{\textit{chuỗi cung ứng phần mềm}} trở thành mục tiêu đối với tác nhân tấn công.
...
\begin{tikzpicture}[
  ...
  box/.style={
    draw, rounded corners=3pt, minimum height=15mm, text width=3.25cm,
    align=center, inner sep=4pt, line width=0.65pt
  },
  pip/.style={box, fill=blue!7, draw=blue!55},
  pdp/.style={box, fill=orange!11, draw=orange!65},
  pep/.style={box, fill=green!8, draw=green!55},
  obs/.style={box, fill=violet!8, draw=violet!55},
  flow/.style={->, >=Latex, line width=0.75pt, draw=gray!65},
  fb/.style={->, >=Latex, dashed, line width=0.75pt, draw=gray!65},
  role/.style={font=\scriptsize\sffamily\bfseries, text=gray!70}
]
...
\node[pip, right=of id] (posture) {
  \textbf{2. Tư thế bảo mật}\\
  lỗ hổng, cấu hình\\
  tình báo đe dọa
};

\coordinate (pipmid) at ($(id)!0.5!(posture)$);
\node[pdp, below=13mm of pipmid] (decision) {
  \textbf{3. Quyết định}\\
  đánh giá rủi ro\\
  phân phối chính sách
};

\node[pep, below=13mm of decision] (enforce) {
  \textbf{4. Thực thi}\\
  biên, mạng, admission\\
  runtime, bí mật động
};

\node[obs, right=of decision] (observe) {
  \textbf{5. Quan sát}\\
  nhật ký, luồng mạng\\
  phản hồi thích ứng
};

\node[role, above=2mm of id] {PIP};
\node[role, above=2mm of posture] {PIP};
\node[role, left=2mm of decision] {PDP};
\node[role, left=2mm of enforce] {PEP};
\node[role, above=2mm of observe] {PIP};

\draw[flow] (id.south) |- (decision.west);
\draw[flow] (posture.south) |- (decision.north);
\draw[flow] (decision) -- (enforce);
\draw[flow] (enforce.east) -| (observe.south);
\draw[fb] (observe.west) -- node[above, font=\scriptsize\sffamily, text=gray!65]
  {ngữ cảnh mới} (decision.east);

\end{tikzpicture}
\caption{Kiến trúc Zero Trust đề xuất cho hệ thống Microservices trên Kubernetes}
...
Năm thành phần trên không hoạt động độc lập mà phối hợp theo một luồng xử lý tuần tự mỗi khi có yêu cầu truy cập. Hình~\ref{fig:zta_five_components} mô tả luồng này qua sáu bước khép kín.
