...
\section{Triển khai Zero Trust trong hệ thống job7189}
\subsection{Kiến trúc tổng thể và phân vùng an ninh}
Triển khai Zero Trust trong job7189 được tổ chức theo các namespace chuyên biệt, nhằm giảm bề mặt tấn công và cô lập miền rủi ro. Các lớp hạ tầng cốt lõi bao gồm: \textit{kube-system} (Cilium, Hubble), \textit{security} (Keycloak, Vault, cert-manager), \textit{gateway} (Kong), \textit{data} (MySQL, Redis, Kafka, MinIO), và \textit{logging} (ELK, Sentry). Cách phân vùng này bảo đảm mỗi lớp có chính sách truy cập độc lập, phù hợp với nguyên tắc \textit{Never Trust, Always Verify} \cite{rose2020zero}.

\subsection{Lớp mạng: Cilium, eBPF và mTLS sidecarless}
Cilium sử dụng eBPF để thực thi Network Policy L3/L4/L7 trực tiếp ở tầng kernel, giúp lọc lưu lượng nội bộ theo danh tính service, phương thức HTTP và đường dẫn API \cite{cilium_docs}. Cấu hình \textit{deny-all/allow-explicit} được ưu tiên, đảm bảo mỗi luồng \textit{East-West} đều phải được khai báo rõ ràng. Trên nền tảng này, mTLS sidecarless được kích hoạt để mã hóa và xác thực hai chiều giữa các service, loại bỏ giả định tin cậy nội bộ.

Khái niệm \textbf{tính trong suốt (transparency)} trong kiến trúc service mesh được hiện thực hóa tại lớp nền tảng: cơ chế TLS, retry, rate limit và observability được cung cấp ở mức hạ tầng, vì vậy lập trình viên chỉ tập trung vào \textit{business logic} thay vì tự triển khai từng cơ chế bảo mật trong từng dịch vụ. Điều này làm giảm đáng kể sai sót cấu hình và tăng tính nhất quán bảo mật trên toàn hệ thống.

\subsection{Định danh và bí mật: Keycloak + HashiCorp Vault}
Hệ thống sử dụng Keycloak làm IdP theo chuẩn OIDC, phát hành JWT cho người dùng và dịch vụ nội bộ \cite{keycloak_docs}. Ở tầng dữ liệu, HashiCorp Vault giải quyết bài toán \textit{Secret Zero} bằng cách cấp phát \textit{dynamic credentials} cho MySQL với TTL hữu hạn, đồng thời tự động thu hồi khi hết hạn \cite{vault_docs}. Cơ chế Vault Agent Injector gắn vào Pod giúp lấy bí mật theo danh tính ServiceAccount, giảm rủi ro rò rỉ trong Kubernetes Secret truyền thống.

\begin{figure}[H]
    \centering
    \textit{Vẽ sequence diagram cho luồng xác thực Vault Dynamic Secrets: Pod (ServiceAccount) \textrightarrow Vault Auth \textrightarrow Database Secrets Engine \textrightarrow Vault Agent render vào /vault/secrets.}
    \caption{Luồng cấp phát Dynamic Secrets qua Vault trong Kubernetes}
    \label{fig:vault_dynamic_secrets}
\end{figure}

\subsection{Gateway: Kong và Cilium Gateway API}
Kong Gateway chịu trách nhiệm kiểm tra JWT, áp chính sách xác thực tập trung và định tuyến lưu lượng \textit{North-South} vào các dịch vụ nội bộ \cite{kong_docs}. Song song, Cilium Gateway API đảm nhiệm chức năng routing tại tầng hạ tầng mạng, cho phép hợp nhất mô hình điều phối lưu lượng với Network Policy và mTLS, từ đó giảm sự phân mảnh chính sách giữa gateway và lớp mạng.

\subsection{Hạ tầng dưới dạng mã: Helmfile}
Hạ tầng được triển khai theo mô hình \textit{Infrastructure as Code} với Helmfile, cho phép quản lý đồng bộ cấu hình Cilium, Vault, Keycloak và các microservices theo môi trường \cite{helmfile_docs}. Cách tiếp cận này đảm bảo khả năng tái lập, kiểm soát thay đổi và giảm lỗi cấu hình thủ công trong các bước triển khai.

\subsection{Observability và phản ứng an ninh}
Hubble cung cấp telemetry ở tầng mạng (flow logs, service map), trong khi ELK và Sentry thu thập nhật ký và lỗi ứng dụng ở tầng nghiệp vụ \cite{cilium_docs,elk_docs}. Chuỗi quan sát này hỗ trợ phát hiện sớm hành vi bất thường, củng cố nguyên tắc giám sát liên tục trong ZTA.

\begin{figure}[H]
    \centering
    \textit{Vẽ sơ đồ kiến trúc observability: Cilium/Hubble \textrightarrow Filebeat/Logstash \textrightarrow Elasticsearch \textrightarrow Kibana, kèm nhánh cảnh báo tới Alerting.}
    \caption{Chuỗi quan sát và phân tích an ninh tập trung}
    \label{fig:zta_observability}
\end{figure}
