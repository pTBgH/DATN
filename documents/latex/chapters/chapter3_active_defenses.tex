...
\begin{itemize}[itemsep=4pt, parsep=0pt]
    ...
    \begin{itemize}
        ...
        \item \textit{Cilium Network Policy:} Chính sách mạng cho phép lưu lượng đi ra từ Analyzer tới dịch vụ DNS và dịch vụ Keycloak nội bộ, đồng thời mở lưu lượng đi vào Keycloak để tiếp nhận kết nối an toàn.
    \end{itemize}
    \item \textbf{Tại sao cấu hình như thế:} Cơ chế Impossible Travel được thiết kế ở chế độ Shadow (Audit-only) để chạy thử nghiệm và kiểm toán chất lượng dữ liệu trước khi chuyển sang giai đoạn cưỡng chế. Cách vận hành này giúp giảm nguy cơ khóa nhầm tài khoản của người dùng hợp lệ do thuật toán nhận diện sai địa chỉ IP động.
...
\begin{itemize}[itemsep=4pt, parsep=0pt]
    ...
    \end{itemize}
    \item \textbf{Tại sao cấu hình như thế:} Thiết kế ma trận phân cấp bảo vệ giúp hệ thống không phản ứng thái quá bằng cách xoá sổ Pod (làm gián đoạn dịch vụ tức thời), mà thay vào đó là tước đoạt dần quyền truy cập mạng nội bộ. Khi Pod bị hạ điểm về mức \texttt{low}, chính sách mạng sử dụng luật chặn lưu lượng đi ra \texttt{egressDeny} để ngắt kết nối E-W tới Vault và cơ sở dữ liệu nhằm ngăn chặn tin tặc lấy trộm dữ liệu, nhưng vẫn giữ kết nối North-South để phục vụ việc chẩn đoán.
    \item \textbf{Ý nghĩa hoạt động và kết quả lệnh:} 
    \begin{itemize}
        ...
        \item Lệnh thứ hai (mục 2) khởi chạy quy trình tự động tạo các Pod giả lập có nhãn tin cậy khác nhau để thử nghiệm kết nối tới \texttt{Vault}, kết quả ghi nhận Pod \texttt{low} bị chặn kết nối trong khi Pod \texttt{high} kết nối thành công (\texttt{low=blocked high=reachable}).
