...
Bề mặt tấn công (Attack Surface): Lỗ hổng logic ứng dụng (chưa được công bố - Zero-day hoặc lỗi Deserialization) trên Microservices hiring-service (Dịch vụ xử lý hồ sơ tuyển dụng, có API public tiếp nhận tệp tin CV từ Internet).
...
Bề mặt tấn công: Hệ thống biến môi trường và vùng nhớ tạm (RAM Disk) bên trong Microservices job-service. Giả định tác nhân đe dọa đã thiết lập được chỗ đứng ngầm (foothold) thông qua lỗ hổng ứng dụng từ Kịch bản 2.
