...
Watches Pod label changes in 7 ZTA namespaces and:
  ...
  3. Computes weighted trust score from 4 inputs (labels, CVE, tier, KEV):
     score = max(0, 100 - 30*(missing_labels/6) - 50*has_critical
                  - 20*has_high - WEIGHT_KEV*has_kev - tier_penalty)
     where tier_penalty = round(15 * TIER_FACTOR[tier]) only when the pod
     already has a posture issue (missing label or CVE), and has_kev is true
     when one of the image's CVEs is on the CISA Known-Exploited list. The
     KEV weight defaults to 0 (inert) until the kev-catalog ConfigMap is
     populated and PDP_WEIGHT_KEV is set.
...
# Exploitability axis (CISA KEV). A CVE that is on the Known-Exploited list is
# far more dangerous than a merely "critical"-scored one. has_kev implies the
# pod already has a CVE, so this never penalises a clean pod. The weight
# defaults to 0 (inert) so deploying this controller does NOT change any bucket
# until you (a) populate the kev-catalog ConfigMap (06-pdp-kev-score.sh) and
# (b) set PDP_WEIGHT_KEV to a positive value on the Deployment.
WEIGHT_KEV = int(os.environ.get("PDP_WEIGHT_KEV", "0"))
# Where the KEV catalogue (CISA known_exploited_vulnerabilities) lives: a
# ConfigMap with one data key whose value is newline-separated CVE IDs.
KEV_CM_NAMESPACE = os.environ.get("PDP_KEV_CM_NAMESPACE", "security")
KEV_CM_NAME = os.environ.get("PDP_KEV_CM_NAME", "kev-catalog")
KEV_CM_KEY = os.environ.get("PDP_KEV_CM_KEY", "kev.txt")
...
def _refresh_vr_cache(dyn_client: DynamicClient) -> None:
    ...
    try:
        ...
        new_ids: dict[str, dict[str, set[str]]] = {}
        for item in reports.items:
            ...
            ids = new_ids[ns].setdefault(pod_name, set())
            for v in (getattr(item.report, "vulnerabilities", None) or []):
                vid = getattr(v, "vulnerabilityID", "") or ""
                if vid:
                    ids.add(vid)
        _vr_cache = new_cache
        _vr_ids_cache = new_ids
        _vr_cache_ts = now
    except Exception as exc:
        log.warning(f"VulnerabilityReport fetch failed: {exc}")


def _refresh_kev_set(api: client.CoreV1Api) -> None:
    """Load the CISA KEV CVE-ID set from the kev-catalog ConfigMap (cached).

    Fully graceful: if the ConfigMap is absent or unreadable the set stays
    empty, so has_kev() is always False and the KEV axis has zero effect.
    """
    global _kev_set, _kev_cache_ts
    if WEIGHT_KEV <= 0:
        return
    now = time.time()
    if _kev_set and now - _kev_cache_ts < _KEV_CACHE_TTL:
        return
    try:
        cm = api.read_namespaced_config_map(KEV_CM_NAME, KEV_CM_NAMESPACE)
        raw = (cm.data or {}).get(KEV_CM_KEY, "")
        _kev_set = {line.strip() for line in raw.splitlines() if line.strip()}
        _kev_cache_ts = now
    except Exception as exc:
        log.warning(f"KEV catalog fetch failed: {exc} — KEV axis inert")
        _kev_set = set()
        _kev_cache_ts = now


def pod_has_kev(ns: str, pod_name: str, owner_name: str) -> bool:
    """True if any CVE on the pod's image is in the cached CISA KEV set."""
    if WEIGHT_KEV <= 0 or not _kev_set:
        return False
    ns_ids = _vr_ids_cache.get(ns, {})
    ids: set[str] = set()
    if owner_name and owner_name in ns_ids:
        ids = ns_ids[owner_name]
    elif pod_name in ns_ids:
        ids = ns_ids[pod_name]
    else:
        for key, val in ns_ids.items():
            if pod_name.startswith(key) or key.startswith(
                pod_name.rsplit("-", 1)[0]
            ):
                ids = val
                break
    return bool(ids & _kev_set)
...
def on_startup(settings: kopf.OperatorSettings, **_kwargs):
    ...
    api = client.CoreV1Api()
    if WEIGHT_KEV > 0:
        audit("kev-axis-enabled", weight=WEIGHT_KEV,
              catalog=f"{KEV_CM_NAMESPACE}/{KEV_CM_NAME}")
