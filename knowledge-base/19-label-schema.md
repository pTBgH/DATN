...
| `hubble-flow-shipper` | monitoring | scraper | T2 | prod | internal | cluster-only | platform |
...
| `minio` | data | db | T2 | prod | internal | internal | data |
...
> **Cập nhật 2026-06-30:** `minio` đã deploy trong namespace `data` và dùng label
> schema ở trên. `docker-registry` không chạy in-cluster; registry thật chạy host-level
> trên `baosrc`.
...
> ZTA từ 2026-05 (xem `knowledge-base/cleanup-workspace-plan.md`, PR-C). Code dev cục
...
- **KHÔNG xếp tier theo cảm tính**: tier chỉ căn cứ DAAS classification chính thức (`knowledge-base/18-daas-classification.md`).
