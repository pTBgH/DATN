...
**Decision đi cùng:** `knowledge-base/zta-gap-decision.md` — chốt các câu hỏi ở §9; mọi PR `PR-I`..`PR-M` đều tham chiếu file đó.
...
| 2 | **CDM (PIP 4)** | chap1 §1.2.2, chap2 §2.3.2 (Posture/Threat Intel layer) | 0 deployment; `knowledge-base/16-pip-data-sources.md` tự thừa nhận ❌/⚠️ | ❌ Missing |
...
| 9 | **Tetragon → PDP closed loop** | `knowledge-base/24-adaptive-security-loop.md` §6: kill event → giảm trust → policy update | PDP chỉ watch label, **không subscribe Tetragon events** (`pdp_controller.py` chỉ có `kopf.on.create/update` cho Pod) | ⚠️ Open loop |
...
### Bằng chứng
- `knowledge-base/16-pip-data-sources.md` §"PIP 4: CDM" tự đánh dấu **⚠️ Partial — Trivy manual, kube-bench chưa deploy**.
