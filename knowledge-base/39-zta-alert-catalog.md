...
Three tiers of detection are described in `knowledge-base/24-adaptive-security-loop.md`:
...
|---|---|---|---|---|
| `ZTATrustScoreDropped` | warning | T1078 | `pdp_trust_score < 60` for 2m | `knowledge-base/25-pdp-controller.md` |
| `ZTACVECriticalImage` | critical | T1190 | `pdp_cve_critical_total > 0` for 1m | `knowledge-base/zta-gap-decision.md` |
| `ZTALabelDriftHigh` | warning | — | label drift rate > 5/min for 5m | `knowledge-base/19-label-schema.md` |
...
| Alert | Severity | MITRE | Triggers on | Runbook |
|---|---|---|---|---|
| `ZTACiliumDropsHigh` (PR-L) | warning | — | per-reason drop rate > 10/s for 5m | `knowledge-base/05-observability-stack.md` |
| `ZTACiliumCrossTierDropZScore` (**Tier 2**) | warning | T1071 | per-(src,dst)-ns drop z-score > 3 sigma for 10m | `knowledge-base/34-microseg-phase2c-plan.md` |
| `ZTAHubbleDNSExfilSuspect` (PR-O) | warning | T1071 | per-pod rate of DNS queries with subdomain length ≥ 60 > 0.2/s for 10m | `knowledge-base/30-hubble-flow-sink.md` |
| `ZTAHubbleEgressToThreatCIDR` (Phase 3) | critical | T1041 | sustained egress drop with `reason="Policy denied"`, `traffic_direction="EGRESS"`, `destination=~".*reserved:(world\|unknown).*"` for 2m | `knowledge-base/24-adaptive-security-loop.md` |
...
| Alert | Severity | MITRE | Triggers on | Runbook |
|---|---|---|---|---|
| `ZTAKongRequestRateZScore` (**Tier 2**) | warning | T1190 | per-service request z-score > 3 sigma for 10m | `knowledge-base/05-observability-stack.md` |
| `ZTAKong5xxRateHigh` | critical | T1190 | per-service 5xx ratio > 5% for 5m | `knowledge-base/05-observability-stack.md` |
| `ZTAKongLatencyP95High` | warning | — | per-service p95 latency > 1000ms for 5m | `knowledge-base/05-observability-stack.md` |
| `ZTAKongJWTAuthFailureSpike` | warning | T1078 | per-consumer 401-response rate > 10/min for 5m | `knowledge-base/api-authentication-guide.md` |
...
no `function` / `file` / `policy` labels (see knowledge-base/14-tetragon-runtime.md).

| Alert | Severity | MITRE | Triggers on | Runbook |
|---|---|---|---|---|
| `ZTATetragonEventRateZScore` (**Tier 2**) | warning | T1059 | per-workload event z-score > 3 sigma for 10m | `knowledge-base/14-tetragon-runtime.md` |
| `ZTATetragonShellExec` (Phase 3 tuned) | critical | T1059 | per-workload shell-binary exec rate > 0.1/s for 2m | `knowledge-base/14-tetragon-runtime.md` |
| `ZTATetragonShellExecBurst` (Phase 3 new) | critical | T1059 | per-workload shell-binary exec rate > 1/s for 1m | `knowledge-base/14-tetragon-runtime.md` |
| `ZTATetragonSensitiveFileRead` | critical | T1556 | any PROCESS_KPROBE event in `job7189-apps` (sensitive-file / suspicious-exec policies) | `knowledge-base/14-tetragon-runtime.md` |
| `ZTATetragonKernelModuleLoad` | critical | T1611 | host-namespace PROCESS_KPROBE event from a non-kubelet/containerd binary | `knowledge-base/14-tetragon-runtime.md` |

### Group `zta-cdm-trust` (PR-O)

| Alert | Severity | MITRE | Triggers on | Runbook |
|---|---|---|---|---|
| `ZTACosignAdmissionDenied` | critical | T1543 | per-ns rate of `policy_controller_admission_decisions_total{decision="deny"}` > 0 for 2m | `knowledge-base/28-sigstore-policy-controller.md` |
| `ZTASpireAttestFailureRate` | warning | T1078 | per-instance `spire_server_node_attestor_failure_total` rate > 0.2/s for 5m | `knowledge-base/27-spire-workload-attestation.md` |
| `ZTACertExpirySoon` | warning | — | `certmanager_certificate_expiration_timestamp_seconds - time() < 7 days` for 1h | `knowledge-base/15-encryption-mtls-spiffe.md` |
