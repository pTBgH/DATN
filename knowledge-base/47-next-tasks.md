...
**Vấn đề:** SPIRE server + agents đang Running, đã verify workload lấy được SVID.

**Việc cần làm:**
- [x] Check SVID issuance: `kubectl exec -n spire spire-server-0 -- /opt/spire/bin/spire-server entry show`
- [x] Verify từ app pod: `kubectl exec -n job7189-apps deployment/identity-service -c app -- ls /run/spiffe/`
- [x] Xác nhận SPIRE agent registration thành công.
...
**Việc cần làm:**
- [x] §3.3: Confirm Tetragon caveat footnote (kernel BPF incompatibility, v1.7.0 fix)
- [x] §5.2: Thêm số liệu latency mới sau khi Item J hoàn thành
- [x] §7 Limitations: Review và cập nhật các row theo trạng thái thực tế 2026-06-12
- [x] Commit và push lên origin

### 9. Chuẩn bị evidence cho bảo vệ luận văn ✅
**Việc cần làm:**
- [x] Chụp Hubble UI showing live flows với policy enforcement
- [x] Screenshot Keycloak OIDC flow hoạt động đầu-cuối (login → token → API)
- [x] Export `kubectl get cnp -A` — tổng hợp toàn bộ policies đang enforce
- [x] Screenshot Tetragon Sigkill working (v1.7.0)
- [x] Gatekeeper admission denial example

### 10. Cleanup duplicate ingress ✅
**Vấn đề:** Đã xóa bỏ ingress trùng lặp trong namespace `security`.

**Việc cần làm:**
- [x] Xóa `ingress-auth-internal`: `kubectl delete ingress -n security ingress-auth-internal`
- [x] Giữ `ingress-keycloak` với `proxy-buffer-size: 128k` (đúng cho Keycloak JWT headers)
...
| SPIRE | spire | ✅ Running | Server + agents, SVID verified |
...
| [`12-security.yaml`](../infras/k8s-yaml/cilium-policies/namespaces/12-security.yaml) | Security namespace policy |
| [`45-upgrade-and-rollback-plan.md`](./45-upgrade-and-rollback-plan.md) | Chi tiết deploy/rollback từng Tier |
| [`46-cilium-l7-same-node-loopback.md`](./46-cilium-l7-same-node-loopback.md) | Root cause L7 timeout |
| [`archive/37-phase5d-followup-todo.md`](./archive/37-phase5d-followup-todo.md) | Thesis chapter4 items (đã lưu trữ) |
