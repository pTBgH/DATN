...
Hệ thống được chạy trên **4 máy ảo (VMs) Ubuntu 22.04 LTS** kết nối qua mạng cục bộ (LAN) và thiết lập mạng mesh VPN mã hóa bằng **Tailscale Overlay**.

```mermaid
graph TD
    subgraph LAN [Mạng LAN Nội bộ - 172.16.82.0/24]
        srv01["7189srv01 (Control Plane)<br>LAN: 172.16.82.131<br>Tailscale: 100.108.231.127"]
        srv02["7189srv02 (Worker 1)<br>LAN: 172.16.82.132<br>Tailscale: 100.113.51.68"]
        srv03["7189srv03 (Worker 2)<br>LAN: 172.16.82.133<br>Tailscale: 100.74.189.43"]
        srv05["7189srv05 (Worker 3)<br>LAN: 172.16.82.128<br>Tailscale: 100.105.127.112"]
    end
...
[Cloudflare Edge Node] (https://precise-boolean-sprint.trycloudflare.com)
      │
      ▼ (Cloudflared Tunnel)
[7189srv02 Host (100.113.51.68)] -- cổng tunnel dịch sang localhost:18000
      │
      ▼ (Port-Forward / DNAT)
...
3.  **Cross-Node eBPF Tunneling**: Kong Gateway thực hiện phân giải route `/api/public/jobs` và chuyển tiếp đến `job-service`.
    ...
    *   Gói tin rời card mạng Tailscale của `srv02` (IP `100.113.51.68`) sang card mạng Tailscale của `srv03` (IP `100.74.189.43`) qua kênh WireGuard được mã hóa hoàn toàn.
