...
### 3. Tầng Giao Diện Người Dùng (Frontend Layer) — ⚠️ ĐÃ TÁCH KHỎI CLUSTER ZTA
> **Cập nhật 2026-06-03:** Frontend (Next.js) **không còn** deploy trong cluster ZTA.
> Namespace `frontend` **không tồn tại** trên cluster (đã xác nhận qua reconcile).
> FE phát triển/triển khai tách riêng (xem `knowledge-base/19-*` và
> `knowledge-base/cleanup-workspace-plan.md`). Bảng dưới chỉ giữ làm tham chiếu lịch sử
> cho thiết kế ban đầu (`fe-candidate`/`fe-recruiter`), KHÔNG phản ánh trạng thái runtime.

| Service Name (lịch sử) | Namespace (không còn) | Cổng | Ghi chú |
| :--- | :--- | :--- | :--- |
| ~~`fe-candidate`~~ | ~~`frontend`~~ | ~~3000~~ | (Lịch sử) Giao diện cho Ứng viên — nay deploy ngoài cluster. |
| ~~`fe-recruiter`~~ | ~~`frontend`~~ | ~~3000~~ | (Lịch sử) Giao diện cho Nhà tuyển dụng — nay deploy ngoài cluster. |
...
> ⚠️ **MinIO (object store cho `storage-service`) HIỆN CHƯA DEPLOY** trên cluster ZTA
> (không có `deploy/sts minio` ở ns `data` hay `job7189-infra`; snapshot 40 không nhắc MinIO).
> Manifest tham chiếu: `infras/.../minio-deploy.yaml` (xem `cleanup-workspace-plan.md`).
> `storage-service` vẫn chạy nhưng luồng lưu file qua MinIO chưa khả dụng cho tới khi deploy.
...
| `storage-service` | `job7189-apps`| **80** | Dịch vụ trung gian quản lý File/Object/CV. Backend object store **MinIO CHƯA DEPLOY** trên cluster (xác nhận 2026-06-03) — xem ghi chú dưới. |
