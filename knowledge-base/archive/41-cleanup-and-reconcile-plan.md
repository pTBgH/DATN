...
> Gồm 2 phần: **A. Dọn dẹp & đổi tên `knowledge-base/` → `knowledge-base/`** (cần duyệt vì
...
- Chỉ **gom các file lẻ ở ROOT** vào đúng chỗ; xoá rác/nhị phân; đổi `knowledge-base/` →
  `knowledge-base/`.
...
| `doc.zip` | Bản nén của chính `knowledge-base/` — thừa |
...
### A.5 — ĐỔI TÊN `knowledge-base/` → `knowledge-base/`
```bash
git mv doc knowledge-base
```
**Phải cập nhật tham chiếu `knowledge-base/`** trong **58 file** (đã đếm) ngoài thư mục
`knowledge-base/` + `documents/` (README.md, các `*.sh`, `infras/**`, `k8s-management/**`).
Cách an toàn:
```bash
# 1) Xem trước
grep -rIl 'knowledge-base/' --include='*.md' --include='*.sh' --include='*.yaml' --include='*.yml' . \
  | grep -vE '/(documents)/'
# 2) Thay (BACKUP/branch trước). Chỉ thay tiền tố 'knowledge-base/<NN|chữ>' → 'knowledge-base/'
grep -rIl 'knowledge-base/' . | grep -vE '\.git/' \
  | xargs sed -i 's#\bdoc/#knowledge-base/#g'
```
> ⚠️ Cẩn thận false-positive: `sed` trên có thể đụng chuỗi như `…/knowledge-base/` trong
> ngữ cảnh khác. Phần thực thi sẽ review từng file qua `git diff` trước khi commit,
> và cập nhật cả các liên kết nội bộ trong chính KB (vd `knowledge-base/15-…` → `knowledge-base/15-…`).
...
5. Cập nhật `README.md` (link `knowledge-base/README.md` → `knowledge-base/README.md`).
...
Khi 2 chương KB mâu thuẫn, script check theo **`knowledge-base/40-snapshot-20260527`** (snapshot
...
| # | Lệch | Chi tiết |
|---|---|---|
| 1 | ES version | knowledge-base/05 ghi *8.x*, snapshot 40 ghi *7.17.18* |
| 2 | Encryption | knowledge-base/04 ghi *mTLS/WireGuard ĐÃ BẬT*, snapshot 40 ghi *mesh-auth DISABLED* |
| 3 | Hạ tầng | knowledge-base/06 còn nói *Kind, ~12GB*; snapshot 40 nói *multi-VM kubeadm, 32GB* |
| 4 | Cluster type | knowledge-base/00 + knowledge-base/08 còn vẽ *Kind 1CP+3W*; thực tế *kubeadm srv01..05* |
| 5 | Frontend | port-mapping doc liệt kê ns `frontend`; knowledge-base/19 nói *FE đã tách khỏi cluster* |
...
| 3 | Redis cache | `<svc>-redis` mỗi service (knowledge-base/19) |
| 4 | Identity | Keycloak dual-realm, Vault dual + unseal, SPIRE server/agent, PDP `PDP_CVE_INPUT=false` |
| 5 | ServiceAccount | 7 SA 1:1 (knowledge-base/03) |
...
| 8 | Encryption | `cilium-config` mesh-auth/wireguard (in DRIFT vs knowledge-base/04) |
...
| 12 | Label schema | đủ 6 `zta.job7189/*` trên 26 workload (knowledge-base/19) |
