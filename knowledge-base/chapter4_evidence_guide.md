...
| `knowledge-base/40-zta-system-snapshot-20260527.md` | Snapshot 27/05. Dùng cho phần mô tả môi trường & ghi nhận trạng thái rollout. |
...
3. **Cấu hình role (lấy từ knowledge-base/03-secret-management.md hoặc helm values)** — KHÔNG có trong evidence runtime, lấy từ kho config:
   ```
...
1. 1 sơ đồ TikZ (hoặc PNG) cho vòng feedback: `Trivy VR → PDP score → annotation+label → CNP enforce`. Có thể tận dụng hình đã vẽ trong `knowledge-base/architecture/`.
...
warm-path < 5ms theo knowledge-base/24" — nếu có file đó), KHÔNG bịa số P50/P95/P99.
...
- [ ] KHÔNG bịa: bất kỳ con số/log nào không truy được trong `evidence/*` hoặc `knowledge-base/40-*` đều phải bị xoá hoặc chuyển sang §4.6 limitations.
