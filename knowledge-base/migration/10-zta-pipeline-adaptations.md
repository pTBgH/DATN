...
if [ "$ZTA_CLUSTER_MODE" = "external" ]; then
  ...
  echo "(xem knowledge-base/migration/05-storage-and-registry.md §4)"
else
...
Doc `knowledge-base/15-encryption-mtls-spiffe.md` cần thêm chú thích:
...
plan. PR này chỉ chứa `knowledge-base/migration/*.md`, KHÔNG đụng `scripts/`,
