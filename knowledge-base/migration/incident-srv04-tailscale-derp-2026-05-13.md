...
- `knowledge-base/migration/scripts/onboard-srv05.sh` — wrapper around `bootstrap.sh
  --server=05`. Refuses to run if local IP is in `192.168.122.0/24` (the
  whole point — don't re-create the same problem on srv05).
- `knowledge-base/migration/scripts/decommission-srv04.sh` — cordon + drain +
  delete the dead node. Safe to run from srv01.
- `knowledge-base/migration/transition-srv04-to-srv05.md` — step-by-step runbook.
