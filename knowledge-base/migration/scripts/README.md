...
> Đây là **toolkit thực thi** kèm `knowledge-base/migration/01..13-*.md`. Mọi thứ
...
cd ~/projects/DATN
cp knowledge-base/migration/scripts/config.env.example knowledge-base/migration/scripts/config.env
nano knowledge-base/migration/scripts/config.env          # set TAILNET_DOMAIN, TS_AUTHKEY...

# Sau đó MỖI VM chỉ chạy ĐÚNG 1 lệnh:
sudo -E bash knowledge-base/migration/scripts/bootstrap.sh --server=01 --yes   # trên 7189srv01
sudo -E bash knowledge-base/migration/scripts/bootstrap.sh --server=02 --yes   # trên 7189srv02
sudo -E bash knowledge-base/migration/scripts/bootstrap.sh --server=03 --yes   # trên 7189srv03
sudo -E bash knowledge-base/migration/scripts/bootstrap.sh --server=04 --yes   # trên 7189srv04
```
...
bash knowledge-base/migration/scripts/bootstrap.sh --list
bash knowledge-base/migration/scripts/bootstrap.sh --server=01 --list
bash knowledge-base/migration/scripts/bootstrap.sh --server=02 --dry-run --yes

# Chỉ cài lại Cilium trên srv01 (cluster đã up):
sudo -E bash knowledge-base/migration/scripts/bootstrap.sh --server=01 --phase=cilium --yes

# Resume sau khi host-prep + control-plane đã xong, chạy cilium tiếp:
sudo -E bash knowledge-base/migration/scripts/bootstrap.sh --server=01 --from=cilium --yes

# Bỏ qua cluster-services lần này (sẽ chạy sau, ví dụ sau khi all workers joined):
sudo -E bash knowledge-base/migration/scripts/bootstrap.sh --server=01 --skip=cluster-services --yes
...
sudo -E bash knowledge-base/migration/scripts/bootstrap.sh --server=02 --yes
```
...
```
knowledge-base/migration/scripts/
...
2. **Forced rollback** sau khi đã hoàn thành phase (nếu cần undo):
   ```bash
   sudo bash knowledge-base/migration/scripts/99-rollback.sh --force --phase=control-plane
   ```
...
bash knowledge-base/migration/scripts/bootstrap.sh --server=01 --dry-run --yes

# 2. ZTA_DRY_RUN=1 inside a phase — pre-flight chạy, command nội bộ log nhưng không thực thi:
sudo ZTA_DRY_RUN=1 -E bash knowledge-base/migration/scripts/bootstrap.sh --server=01 --phase=host-prep --yes
```
...
sudo -E bash knowledge-base/migration/scripts/bootstrap.sh --server=01 --phase=control-plane --yes
```
...
sudo SSH_FETCH=1 -E bash knowledge-base/migration/scripts/bootstrap.sh --server=02 --yes
```
...
```bash
sudo -E bash knowledge-base/migration/scripts/bootstrap.sh --server=04 --yes
...
sudo bash knowledge-base/migration/scripts/99-rollback.sh --force --phase=cluster-services   # admin/srv01
sudo bash knowledge-base/migration/scripts/99-rollback.sh --force --phase=cilium              # admin/srv01
sudo bash knowledge-base/migration/scripts/99-rollback.sh --force --phase=worker-join         # mỗi worker
sudo bash knowledge-base/migration/scripts/99-rollback.sh --force --phase=control-plane       # srv01
sudo bash knowledge-base/migration/scripts/99-rollback.sh --force --phase=host-prep           # mọi VM
```
...
sudo -E bash knowledge-base/migration/scripts/bootstrap.sh --server=01 --phase=host-prep --yes
```
