#!/usr/bin/env bash
# knowledge-base/migration/scripts/bootstrap.sh
#
...
#
#   sudo -E bash knowledge-base/migration/scripts/bootstrap.sh --server=01   # on 7189srv01 (control-plane)
#   sudo -E bash knowledge-base/migration/scripts/bootstrap.sh --server=02   # on 7189srv02 (worker)
#   sudo -E bash knowledge-base/migration/scripts/bootstrap.sh --server=03   # on 7189srv03 (worker)
#   sudo -E bash knowledge-base/migration/scripts/bootstrap.sh --server=04   # on 7189srv04 (worker)
#
# Or auto-detect role from local hostname (recommended once you trust the
# inventory mapping in config.env):
#
#   sudo -E bash knowledge-base/migration/scripts/bootstrap.sh --server=auto
#
...
#   sudo -E bash knowledge-base/migration/scripts/bootstrap.sh --server=01 --yes
#
#   # Skip cluster-services on initial CP bring-up (you'll run it later
#   # after all workers are joined):
#   sudo -E bash knowledge-base/migration/scripts/bootstrap.sh --server=01 \
...
#   bash knowledge-base/migration/scripts/bootstrap.sh --server=02 --dry-run
#
# Logs (per run):
...
if [ "${DRY_RUN}" -ne 1 ] && [ "${LIST_ONLY}" -ne 1 ]; then
  if [ "$(id -u)" -ne 0 ]; then
    ...
    log_err "  Re-run with: sudo -E bash knowledge-base/migration/scripts/bootstrap.sh --server=${SERVER}"
    exit 1
  fi
fi
...
if [ "${OVERALL_RC}" -eq 0 ]; then
  ...
  if [ "${TARGET_ROLE}" = "control-plane" ] && [ -z "${PHASE_FILTER}" ] && [ -z "${TO_PHASE}" ]; then
    ...
    for h in ${WORKER_HOSTNAMES}; do
      ...
      local_num="${h##*[!0-9]}"
      log_info "       sudo -E bash knowledge-base/migration/scripts/bootstrap.sh --server=${local_num}"
    done
