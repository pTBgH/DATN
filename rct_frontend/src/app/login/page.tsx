...
function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await passwordGrant(email.trim(), password);
      router.replace(callbackUrl ?? "/recruiter");
    } catch (err) {
      ...
      <div className="w-full max-w-md bg-white rounded-xl p-8 shadow-lg">
        ...
        <div className="grid grid-cols-1 gap-3">
          <button
            ...
            disabled={loading}
            className="flex items-center justify-center gap-2 py-3 border border-slate-300 rounded-lg text-slate-700 font-medium hover:bg-slate-50 transition disabled:opacity-60"
          >
