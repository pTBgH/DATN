    ...
    <div className="space-y-6">
      ...
      <form className="flex flex-wrap gap-3" method="GET">
        <input
          ...
          className="flex-1 rounded-lg border border-slate-300 px-4 py-2 placeholder-slate-400 focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-100"
        />
        <select
          name="status"
          defaultValue={searchParams.status ?? ""}
          className="rounded-lg border border-slate-300 px-4 py-2 focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-100"
        >
      ...
      <div className="flex items-center justify-between rounded-lg bg-gradient-to-r from-cyan-50 to-blue-50 p-4">
        <p className="text-sm font-medium text-slate-700">
          Tổng <span className="text-cyan-600 font-semibold">{result.meta?.total ?? result.data.length}</span> công việc
        </p>
