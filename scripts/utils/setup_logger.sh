#!/bin/bash
for svc in src/*_service; do
    if [ -d "$svc/laravel_back" ]; then
        echo "Cài đặt StructuredLogger cho $svc..."
        
        # 1. Tạo thư mục App/Support/Logging nếu chưa có
        mkdir -p "$svc/laravel_back/app/Support/Logging"
        
        # 2. Tạo file StructuredLogger.php
        cat << 'LOGEOF' > "$svc/laravel_back/app/Support/Logging/StructuredLogger.php"
<?php
...
LOGEOF
        ...
        # 3. Patch config/logging.php bằng sed (thêm channel structured)
        if ! grep -q "'structured' =>" "$svc/laravel_back/config/logging.php"; then
            sed -i "/'channels' => \[/a \        'structured' => [\n            'driver' => 'single',\n            'path' => storage_path('logs/structured.log'),\n            'level' => env('LOG_LEVEL', 'debug'),\n            'formatter' => \Monolog\Formatter\LineFormatter::class,\n            'formatter_with' => [\n                'format' => \"%message%\\n\",\n            ],\n        ]," "$svc/laravel_back/config/logging.php"
        fi
        
    fi
done
echo "Hoàn thành tạo Logger ở mọi Service!"
